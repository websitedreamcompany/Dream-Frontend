import { Inter } from "next/font/google";
import Image from "next/image";
import Link from "next/link";

const inter = Inter({
  display: "swap",
  weight: "600",
});

const ApplicationPad = () => {
  const cardDetails = [
    {
      name: "Finance App",
      description: "Lorem ipsum dolor sit amet, consectetur adipiscing elit.",
      rating: 4,
    },
    {
      name: "Personnal App",
      description: "Lorem ipsum dolor sit amet, consectetur adipiscing elit.",
      rating: 5,
    },
    {
      name: "Travel App",
      description: "Lorem ipsum dolor sit amet, consectetur adipiscing elit.",
      rating: 3,
    },
  ];

  return (
    <div className="w-full h-full  items-center justify-center">
      {/**Top nav bar */}
      <nav
        className={`${inter.className} text-fh-8 w-screen  fh-95  shadow-f-bottom flex flex-row place-items-center 
                   
                pe-5  `}
      >
        <div className="relative w-[calc(30*var(--spacing-fw))] h-[calc(30*var(--spacing-fw))] ms-fw-16 ">
          <Image
            src="/logo.svg"
            alt="Dream Eco Logo"
            fill
            className="object-contain"
          />
        </div>

        <div className="w-screen justify-end gap-5    flex">
          <Link href="/" className="text-white p-1.5">
            Home
          </Link>

          <Link href="/pages/applications" className=" text-white p-1.5">
            Applications
          </Link>

          <a href="/pages/projects" className="text-white p-1.5">
            Projects
          </a>

          <a href="/pages/shop" className="text-white  p-1.5">
            Shops
          </a>

          <Link
            href={"/pages/account"}
            className="text-white bg-[#800020] w-20 rounded-2xl p-1.5 text-center"
          >
            Login
          </Link>
        </div>
      </nav>

      {/**
       * Main containerr
       */}

      <main className="application h-dvh w-full overflow-y-auto bg-white  ">
        <div className="grid  grid-cols-3 fh-600 bg-gray-600 ">
          <section className="bg-white shadow-f-cardM   p-5   overflow-y-auto g-fit">
            <div className="w-full ">
              <div className="bg-[#2E0B80] fh-40 w-full flex gap-2 ps-5 place-items-center text-white ">
                <input type="checkbox" />
                <p>Alt</p>
              </div>

              <div className="   mt-3 gap-2  pb-3 flex flex-col">
                <div className="flex gap-2">
                  <input type="checkbox" />
                  <p className="text-fw-8">Finance</p>
                </div>

                <div className="flex gap-2">
                  <input type="checkbox" />
                  <p className="text-fw-8">Travel</p>
                </div>
              </div>

              <div className="border-b-[0.5px] border-b-[#D3D3D3] flex flex-col pb-3 gap-2 mt-3 ">
                <div className="flex gap-2">
                  <input type="checkbox" />
                  <p className="text-fw-8">Finance</p>
                </div>

                <div className="flex gap-2">
                  <input type="checkbox" />
                  <p className="text-fw-8">Travel</p>
                </div>
              </div>

              <div className="border-b-[0.5px] border-b-[#D3D3D3] mt-3  pb-3">
                <div className="flex gap-2">
                  <input type="checkbox" />
                  <p className="text-fw-8">Finance</p>
                </div>
              </div>

              <div className="border-b-[0.5px] border-b-[#D3D3D3] mt-3 pb-3">
                <div className="flex gap-2">
                  <input type="checkbox" />
                  <p className="text-fw-8">Finance</p>
                </div>
              </div>

              <div className="border-b-[0.5px] border-b-[#D3D3D3] mt-3 flex flex-col pb-3 gap-3 ">
                <div className="flex gap-2">
                  <input type="checkbox" />
                  <p className="text-fw-8">Finance</p>
                </div>

                <div className="flex gap-2">
                  <input type="checkbox" />
                  <p className="text-fw-8">Travel</p>
                </div>

                <div className="flex gap-2">
                  <input type="checkbox" />
                  <p className="text-fw-8">Finance</p>
                </div>

                <div className="flex gap-2">
                  <input type="checkbox" />
                  <p className="text-fw-8">Finance</p>
                </div>
              </div>

              <div className="border-b-[0.5px] border-b-[#D3D3D3] mt-3 pb-3 ">
                <div className="flex gap-2">
                  <input type="checkbox" />
                  <p className="text-fw-8">Finance</p>
                </div>
              </div>

              <div className="border-b-[0.5px] border-b-[#D3D3D3] mt-3 pb-3">
                <div className="flex gap-2">
                  <input type="checkbox" />
                  <p className="text-fw-8">Finance</p>
                </div>
              </div>
            </div>
          </section>

          <div className="bg-white  ps-20  col-span-2  pt-5 ">
            {/**Card container */}
            <div className="flex  fw-120 flex-col gap-4">
              {cardDetails.map((card, index) => (
                <div
                  key={index}
                  className="fh-200 shadow-f-card  fw-200 bg-white p-5"
                >
                  <div className=" flex gap-4">
                    <div className="fh-30 fw-30">
                      <Image
                        src="/application_contact_icon.png"
                        alt="App 1"
                        width={60}
                        height={60}
                        className="object-contain bg-[#2E0B80]"
                      />
                    </div>

                    <div>
                      <h2 className="text-fw-8 text-[#4C1D60] ">{card.name}</h2>
                      <p className="fh-2 mt-2 fw-80 bg-[#D3D3D3]" />

                      <div className="flex  place-items-center ">
                        <div className="flex gap-2">
                          {Array(5)
                            .fill(0)
                            .map((_, index) => (
                              <div key={index} className="flex gap-2 mt-3 ">
                                <Image
                                  src="/application_rating_star_icon.png"
                                  alt={`App ${index + 1}`}
                                  width={20}
                                  height={20}
                                  className="object-contain "
                                />
                              </div>
                            ))}
                        </div>

                        <p className="">djjjdjdj</p>
                      </div>
                    </div>
                  </div>

                  <div className="mt-5 flex w-full relative ">
                    <div>
                      <p className="text-fw-8">Lorem ipsum dolor</p>
                      <p className="text-fw-8">Lorem ipsum dolor</p>
                    </div>

                    <button className="  fh-40 mt-2 ms-20 bg-[#800020] text-white text-fw-6 px-4 rounded">
                      View Details
                    </button>
                  </div>
                </div>
              ))}
            </div>
          </div>
        </div>

        <div className=" bg-white shadow-f-cardM  flex mt-10  p-5   overflow-y-auto h-300 ">
          <div className="mt-10 ms-3 w-[50%]">
            <div className="">
              <Image
                src="/application_container_3_bg.png"
                alt="App 1"
                width={200}
                height={400}
                className="object-contain "
              />
            </div>

            <div className="flex flex-wrap gap-2 mt-3">
              <div className="flex gap-2 fh-30 ">
                {Array(5)
                  .fill(0)
                  .map((_, index) => (
                    <Image
                      key={index}
                      src="/star_icon.png"
                      alt={`App ${index + 1}`}
                      width={20}
                      height={10}
                      className="object-contain "
                    />
                  ))}
              </div>

              <div className="fw-50 flex flex-col place-items-end">
                <p>Lorem ipsum dolor</p>
                <p>Lorem ipsum dolor</p>
              </div>
            </div>

            <div className="mt-4 flex justify-evenly">
              <p className="text-fw-4 font-bold">
                Lorem ipsum dolor satum vien lori trend{" "}
              </p>
              <p className="text-fw-4 font-bold">Configure</p>
            </div>
          </div>

          <div>
            <div className="mt-5">
              <div className="flex gap-5 m-5">
                <div className="fw-15 fh-60 bg-[#2E0B80] rounded" />

                <div>
                  <p>Lorem ipsum dolor</p>

                  <p>Lorem ipsum dolor</p>
                </div>
              </div>

              <div className="flex gap-5 m-5">
                <div className="fw-15 fh-60 bg-[#2E0B80] rounded" />

                <div>
                  <p>Lorem ipsum dolor</p>

                  <p>Lorem ipsum dolor</p>
                </div>
              </div>
            </div>

            <div className="mt-8">
              <button className="bg-[#4C1D60] fh-50 w-full rounded text-white">
                View details
              </button>

              <button className="bg-[#800020] fh-50 w-full rounded mt-3 text-white">
                DewId New
              </button>
            </div>
          </div>
        </div>
      </main>
    </div>
  );
};

export default ApplicationPad;
